...
import org.finra.dm.app.config.AppSpringModuleConfig;
import org.finra.dm.core.ApplicationContextHolder;
import org.finra.dm.core.config.CoreSpringModuleConfig;
import org.finra.dm.dao.config.DaoEnvSpringModuleConfig;
import org.finra.dm.dao.config.DaoSpringModuleConfig;
import org.finra.dm.rest.config.RestSpringModuleConfig;
import org.finra.dm.service.config.ServiceEnvSpringModuleConfig;
import org.finra.dm.service.config.ServiceSpringModuleConfig;
import org.finra.dm.ui.DmActivitiFilter;
import org.finra.dm.ui.RequestLoggingFilter;
import org.finra.dm.ui.config.UiEnvSpringModuleConfig;
import org.finra.dm.ui.config.UiSpringModuleConfig;
...
public class WarInitializer implements WebApplicationInitializer
{
    ...
    /**
     * Initializes Activiti.
     *
     * @param servletContext the servlet context.
     */
    protected void initActiviti(ServletContext servletContext)
    {
        // Add Activiti filter to serve static resources from /activiti folder
        // for /api, /diagram-viewer, /editor, /explorer, /libs
        FilterRegistration.Dynamic activitiFilter = servletContext.addFilter("DMFilter", DmActivitiFilter.class);
        activitiFilter.addMappingForUrlPatterns(null, false, "/*");

        // Add RequestContextListener To allow session-scoped beans in Spring
        servletContext.addListener(new RequestContextListener());

        // Add Vaadin productionMode=true
        servletContext.setInitParameter("productionMode", "true");

        // Add the VAADIN servlet
        ServletRegistration.Dynamic vaadinservlet = servletContext.addServlet("Vaadin Application Servlet", new ExplorerApplicationServlet());
        vaadinservlet.setInitParameter("widgetset", "org.activiti.explorer.CustomWidgetset");
        vaadinservlet.addMapping("/ui/*");
        vaadinservlet.addMapping("/VAADIN/*");
        vaadinservlet.setLoadOnStartup(10);

        // <!-- Restlet adapter, used to expose modeler functionality through REST -->
        ServletRegistration.Dynamic modelerservlet = servletContext.addServlet("RestletServlet", new ServerServlet());
        modelerservlet.setInitParameter("org.restlet.application", "org.finra.dm.ui.ActivitiExplorerRestApplication");
        modelerservlet.addMapping("/service/*");
        modelerservlet.setLoadOnStartup(11);
    }
}
