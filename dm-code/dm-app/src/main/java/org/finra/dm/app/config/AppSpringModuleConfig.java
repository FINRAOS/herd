...
*/
package org.finra.dm.app.config;
...
import org.finra.dm.app.security.DmUserDetailsService;
...
import org.finra.dm.dao.Log4jOverridableConfigurer;
...
@ComponentScan(value = "org.finra.dm.app",
    excludeFilters = @ComponentScan.Filter(type = FilterType.REGEX, pattern = "org\\.finra\\.dm\\.app\\.config\\..*"))
...
public class AppSpringModuleConfig extends GlobalMethodSecurityConfiguration
{
    ...
    @Autowired
    private DmUserDetailsService dmUserDetailsService;

    /**
     * The Log4J overridable configurer that will handle our Log4J initialization for the DM application.
     * <p/>
    ...
    public static Log4jOverridableConfigurer log4jConfigurer()
    {
        ...
        // Note that the dm-log4j.xml file must be in the dm-war project so it is accessible on the classpath as a file. Placing it within another
        // project's JAR will result in the resource not being found as a "file".
        log4jConfigurer.setDefaultResourceLocation("classpath:dm-log4j.xml");

        // Regardless of which configuration is found, check for an updated configuration at this interval.
        log4jConfigurer.setRefreshIntervalMillis(ConfigurationHelper.getProperty(ConfigurationValue.LOG4J_REFRESH_INTERVAL_MILLIS, Long.class, environment));
    ...
    public AuthenticationManager authenticationManager()
    {
        ...
        authenticationProvider.setPreAuthenticatedUserDetailsService(dmUserDetailsService);
