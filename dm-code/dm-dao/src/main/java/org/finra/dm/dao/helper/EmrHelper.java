...
public class EmrHelper extends AwsHelper
{
    ...
    /**
     * Gets the Oozie DM wrapper Workflow S3 Location ConfigurationValue.
     *
     * @return the ConfigurationValue of S3 location of DM oozie wrapper workflow.
     * @throws IllegalStateException if the S3 location of DM wrapper workflow is not configured.
     */
    public ConfigurationValue getEmrOozieDmWorkflowS3LocationConfiguration() throws IllegalStateException
    {
        String s3HdfsCopyScript = configurationHelper.getProperty(ConfigurationValue.EMR_OOZIE_DM_WRAPPER_WORKFLOW_S3_LOCATION);
        if (StringUtils.isBlank(s3HdfsCopyScript))
        {
            throw new IllegalStateException(String.format(
                "No DM wrapper oozie workflow S3 locaton found. Ensure the \"%s\" configuration entry is configured.",
                ConfigurationValue.EMR_OOZIE_DM_WRAPPER_WORKFLOW_S3_LOCATION.getKey()));
        }

        return ConfigurationValue.EMR_OOZIE_DM_WRAPPER_WORKFLOW_S3_LOCATION;
    }
     ...
     * 
     * @param wrapperWorkflowJob the DM wrapper workflow job.
     * 
