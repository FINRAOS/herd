...
import org.finra.dm.tools.common.databridge.DataBridgeManifestReader;
    ...
    protected void validateManifest(UploaderInputManifestDto manifest) throws IllegalArgumentException
    {
        ...
        if (manifest.getBusinessObjectDataParents() != null)
        {
            ...
            for (BusinessObjectDataKey businessObjectDataKey : manifest.getBusinessObjectDataParents())
            {
                ...
                Assert.isTrue(!parentValidationSet.contains(lowercaseKey),
                    String.format("Duplicate manifest business object data parent found: {%s}", dmHelper.businessObjectDataKeyToString(businessObjectDataKey)));
