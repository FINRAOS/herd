...
public class Hive13DdlGeneratorTest extends AbstractServiceTest
{
        ...
        for (int i = 0; i < badFilePaths.size(); i++)
        {
            ...
            catch (IllegalArgumentException e)
            {
                assertEquals(String.format("Registered storage file or directory does not match the expected Hive sub-directory pattern. " +
                    ...
                    storageFilePaths.get(0), dmDaoHelper.businessObjectDataEntityAltKeyToString(businessObjectDataEntity), TEST_S3_KEY_PREFIX, pattern),
                    e.getMessage());
            }
        }
    }
    ...
    public void testGetHivePartitionsMultiplePathsFound()
    {
        ...
        {
            hive13DdlGenerator
                .getHivePartitions(dmDaoHelper.getBusinessObjectDataKey(businessObjectDataEntity), autoDiscoverableSubPartitionColumns, TEST_S3_KEY_PREFIX,
        ...
        catch (IllegalArgumentException e)
        {
            assertEquals(String.format("Found two different locations for the same Hive partition. " +
                ...
                dmDaoHelper.businessObjectDataEntityAltKeyToString(businessObjectDataEntity), TEST_S3_KEY_PREFIX, partitionPaths.get(0), partitionPaths.get(1)),
                e.getMessage());
        }
    }
