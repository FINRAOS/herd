...
public class ConfigurationHelperTest extends AbstractCoreTest
{
    ...
    {
        ConfigurationValue configurationValue = ConfigurationValue.DM_ENVIRONMENT;
    ...
    public void testGetPropertyThrowsWhenDefaultTypeDoesNotMatch()
    {
        ...
        catch (Exception e)
        {
            ...
            Assert.assertEquals("thrown exception message", "targetType '" + givenType + "' is not assignable from the default value of type '"
                + expectedDefaultType + "'.", e.getMessage());
        }
    }
    ...
    public void testGetPropertyValidationThrowsWhenTargetTypeIsNull()
    {
        ...
        try
        {
            ConfigurationHelper.getProperty(ConfigurationValue.DM_ENVIRONMENT, null, environment);
            Assert.fail("expected IllegalArgumentException, but no exception was thrown");
        }
        catch (Exception e)
        {
