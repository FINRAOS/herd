...
public class MockOozieOperationsImpl implements OozieOperations
{
    ...
    public WorkflowJob getJobInfo(OozieClient oozieClient, String jobId) throws OozieClientException
    {
        ...
        if (CASE_1_JOB_ID.equals(jobId))
        {
            ...
            mockOozieWorkflowJob.setAppName(OozieDaoImpl.HERD_OOZIE_WRAPPER_WORKFLOW_NAME);
        ...
        /*
         * Case5
         * Returns a wrapper workflow with an action which does not correspond to herd wrapper workflow, that is, the client workflow doesn't exist.
         */
        else if (CASE_5_JOB_ID.equals(jobId))
        {
            MockOozieWorkflowJob mockOozieWorkflowJob = new MockOozieWorkflowJob();
            mockOozieWorkflowJob.setId(jobId);
            mockOozieWorkflowJob.setAppName("not_a_herd_wrapper_workflow");
            List<WorkflowAction> workflowActions = new ArrayList<WorkflowAction>();
            {
                MockOozieWorkflowAction workflowAction = new MockOozieWorkflowAction();
    ...
    public List<WorkflowJob> getJobsInfo(OozieClient oozieClient, String filter, int start, int len) throws OozieClientException
    {
        ...
        {
            // Fill 25 herd wrapper with client workflow running
            jobs.addAll(getHerdWrapperJobWithClientRunning(50));

            // Fill 20 herd wrapper with client workflow not running yet
            jobs.addAll(getHerdWrapperJobWithClientNotRunning(40));
    ...
    private List<WorkflowJob> getHerdWrapperJobWithClientRunning(int num)
    {
        List<WorkflowJob> jobs = new ArrayList<>();
        ...
        {
            MockOozieWorkflowJob mockOozieWorkflowJob = new MockOozieWorkflowJob();
            mockOozieWorkflowJob.setId("herd_wrapper_client_running_" + i);
            mockOozieWorkflowJob.setAppName(OozieDaoImpl.HERD_OOZIE_WRAPPER_WORKFLOW_NAME);
            mockOozieWorkflowJob.setStatus(WorkflowJob.Status.RUNNING);
    ...
    private List<WorkflowJob> getHerdWrapperJobWithClientNotRunning(int num)
    {
        List<WorkflowJob> jobs = new ArrayList<>();
        ...
        {
            MockOozieWorkflowJob mockOozieWorkflowJob = new MockOozieWorkflowJob();
            mockOozieWorkflowJob.setId("herd_wrapper_client_not_running_" + i);
            mockOozieWorkflowJob.setAppName(OozieDaoImpl.HERD_OOZIE_WRAPPER_WORKFLOW_NAME);
