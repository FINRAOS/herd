...
public abstract class AwsExceptionRetryAdvice
{
    ...
    @Autowired
    protected HerdThreadHelper herdThreadHelper;
    ...
    public Object retryOnException(ProceedingJoinPoint pjp) throws Throwable
    {
        ...
        long minAwsDelay = herdStringHelper.getConfigurationValueAsInteger(ConfigurationValue.AWS_MIN_RETRY_DELAY_SECS) * 1000L;
        long maxAwsDelay = herdStringHelper.getConfigurationValueAsInteger(ConfigurationValue.AWS_MAX_RETRY_DELAY_SECS) * 1000L;
        ...
        while (true)
        {
            ...
            catch (AmazonServiceException ase)
            {
                ...
                if (RetryUtils.isThrottlingException(ase) || is5xxException(ase) || isRetryableException(ase))
                {
                    ...
                    herdThreadHelper.sleep(nextSleepDelay);
                }
                else
                {
    ...
    {
        List<String> errorCodesToRetry = herdStringHelper.splitStringWithDefaultDelimiter(getExceptionErrorCodes());
