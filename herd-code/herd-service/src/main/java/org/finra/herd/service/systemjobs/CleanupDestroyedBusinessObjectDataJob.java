...
public class CleanupDestroyedBusinessObjectDataJob extends AbstractSystemJob
{
    private static final Logger LOGGER = LoggerFactory.getLogger(CleanupDestroyedBusinessObjectDataJob.class);
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException
    {
        ...
        int maxBusinessObjectDataInstancesToProcess =
            parameterHelper.getParameterValueAsInteger(parameters, ConfigurationValue.CLEANUP_DESTROYED_BDATA_JOB_MAX_BDATA_INSTANCES);

        // Log the parameter values.
        LOGGER.info("systemJobName=\"{}\" {}={}", JOB_NAME, ConfigurationValue.CLEANUP_DESTROYED_BDATA_JOB_MAX_BDATA_INSTANCES,
        ...
        if (maxBusinessObjectDataInstancesToProcess > 0)
        {
            ...
            List<BusinessObjectDataStorageUnitKey> businessObjectDataStorageUnitKeys =
                cleanupDeletedBusinessObjectDataService.getS3StorageUnitsToCleanup(maxBusinessObjectDataInstancesToProcess);

            // Log the number of storage units selected for processing.
            LOGGER
                .info("Selected for processing S3 storage units. systemJobName=\"{}\" storageUnitCount={}", JOB_NAME, businessObjectDataStorageUnitKeys.size());
            ...
            for (BusinessObjectDataStorageUnitKey businessObjectDataStorageUnitKey : businessObjectDataStorageUnitKeys)
            {
                ...
                catch (RuntimeException runtimeException)
                {
                    // Log the exception.
                    LOGGER
                        .error("Failed to cleanup a destroyed business object data. systemJobName=\"{}\" storageName=\"{}\" businessObjectDataKey={}", JOB_NAME,
                            businessObjectDataStorageUnitKey.getStorageName(),
                            jsonHelper.objectToJson(businessObjectDataHelper.createBusinessObjectDataKeyFromStorageUnitKey(businessObjectDataStorageUnitKey)),
                            runtimeException);
                }
            }
        }
        ...
        LOGGER.info("Cleanup destroyed business object data instances. systemJobName=\"{}\" businessObjectDataCount={}", JOB_NAME,
